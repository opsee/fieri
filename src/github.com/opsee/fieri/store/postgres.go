...
func (pg *Postgres) ListInstances(options *Options) ([]*Instance, error) {
	...
	if options.GroupId != "" {
		err = pg.db.Select(&instances, "select * from instances where customer_id = $1 and id in (select instance_id from groups_instances where customer_id = $1 and group_name = $2)", options.CustomerId, options.GroupId)
